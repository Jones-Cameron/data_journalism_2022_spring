...
```{r}
wv_slice <- read_rds("data/lab_05.rds")
...
**A1.**
126 E Burke St has the most loans. There are 6 loans to this street address. Businesses include Drew Holdings LLC, BRIX27, LLC, ABRAHAM ASHTON, HUB CO-OP LLC, HUB CO-OP, LLC, and RONIN PROPERTIES LLC. 
```{r}
cleaned_address <- read_csv("data/lab-05-address-clean.csv")
...
burke_businesses <- cleaned_address %>%
...
**A2.**
The address is "The Hub" a group work space/office space in West Virginia. At first glance, this is not that suspicious as it appears multiple different businesses simply share a work space. However, on a deeper dive I found that all the businesses have a Abraham Ashton  recorded as their orgainzer, and a Robert Johnson is listed as the manager or agent on the majority of the businesses. This is a little suspicious, as the companies all seem to be owned by the same people who would've done the same paperwork for all the businesses. 
...
write_csv(berkeley_county, "data/berkeley_county.csv")
```
...
**A3.**
Martinsburg has the highest number of loans (1166). Full-Service Restaurants have more loans than any other industry title (50).

```{r}
berkeley_county <- wv_slice %>% filter(project_county_name == "BERKELEY")

cleaned_berkeley <- read_csv("data/berkeley_county_cleaned.csv")
...
cleaned_berkeley_with_naics <- cleaned_berkeley %>% left_join(naics_codes,  by="naics_code") 

martinsburg_data <- cleaned_berkeley_with_naics %>% 
...
**A4.** 
One hypotheses is just the shear amount of restaurants in Martinsburg. In my research I could find well over 115 restaurants in Martinsburg and there are only 17,475 people in the city. This means that restaurants may be the most common business in Martinsburg in general, let alone the most common loan. 
...
**A5.**
BAVARIAN INN INC. for $841,010.90. It was approved February 17, 2021 (2021-02-17). 76 jobs retained. Its also known as the Bavarian Inn, Resort and Brewing Company. It has one location. So, I'm assuming I may have the wrong business. Must've missed something in the code. 
...
restaurants_martinsburg <- cleaned_berkeley_with_naics %>% 
  filter (title == "Full-Service Restaurants") %>% 
  arrange(desc(amount))

restaurants_martinsburg
