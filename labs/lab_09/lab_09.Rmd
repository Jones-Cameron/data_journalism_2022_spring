...
```{r}
ppp_wv <- readRDS("data/lab_09.rds")
...
```{r}
ppp_wv_loans_by_date <- ppp_wv %>% 
  group_by(date_approved) %>% 
  summarise(total_loans=n())

april_ppp <- ppp_wv_loans_by_date %>% 
  filter(
  str_detect(date_approved,"2020-04"))
...
april_ppp %>%
  ...
  theme(
    axis.text.x = element_text(angle = 45,  hjust=1)) +
      labs(
    title="West Virginia Approved Loans by Date in April",
    x = "Date",
...
**A1.** April 28 had the most loans approved.
...
```{r}
ppp_states <- read_csv("data/state_ppp_all.csv")

census_api_key("2dab2f9b69151aab34c4c94ddc8a797a04f3944", install=TRUE, overwrite = TRUE)
