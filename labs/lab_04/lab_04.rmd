...
**A1** Taxi Service (7363), Beauty Salons (7275), Full-Service Restaurants (5490), Offices of Real Estate Agents and Brokers (4443), and All Other Personal Services (3956)
...
**A2** Full-Service Restaurants (1488), Offices of Physicians (except Mental Health Specialists) (580), and Plumbing, Heating, and Air-Conditioning Contactors (458)
...
**A3** Zip code 20850 had the largest sum of loans with 474,525,058, and had 3186 (count) loans.
...
**A4** zip code 20850 has the largest sum of loans (28,994,798). The percentage of non-Hispanic Asian population is 23.5 percent, 3,186 (count) loans.
...
**A5** The area is extremely wealthy so many people may not be applying for these PPP loans. Additionally, the people living in ZIP code 20850 are primarily white, so I'm sure the vast majority of loans in that area go to white business owners. Rockville, MD has a large amount of businesses and is a majority white area.  
...
**A6** 3 zip codes, each with 6, 3 and 4 loans respectively. Somoeone put in the wrong information in either the Race category or on the ZCTA end. Either way it's most likely a human error. 

```{r}
# Write the code to produce your answer here
asian_filter <- maryland_ppp_with_naics_zcta %>% 
    filter(race == "Asian")
...
**A7** Fixed it. Executive and legislative offices, combined. Interesting to see what that loan is used for as something for executuve/legislative offices is most likely important. 

```{r}
# Write the code to produce your answer here
one_loan_data <- maryland_ppp_with_naics_zcta %>%
 group_by(title) %>%
 summarise(total=n()) %>%
  arrange(desc(total))
