...
counties_wv <- naics_wv %>% 
  group_by(project_county_name) %>% 
  summarize(total_loans=n(), loan_amount = sum(amount)) %>% 
...
wv_loans_per_100k <- wv_county_population %>% 
  left_join(counties_wv, by=c("COUNTY" = "project_county_name"))
  
wv_loans_per_100k <- wv_loans_per_100k %>% 
...
**A1.** The middle regions of the state seem to be getting the most loans per 100,000 people. Ritchie had the highest loan amount per 100k people. Harrison had the highest total loan amount. They had 86.882252 loans per 100k people, 9,919,762.98 loans total and the population is 67,908.
...
```{r}
shapefile <- st_read("https://tagis.dep.wv.gov/arcgis/rest/services/WVDEP_enterprise/mining_reclamation/MapServer/0/query?where=1%3D1&outFields=*&geometry=&geometryType=esriGeometryEnvelope&inSR=4326&spatialRel=esriSpatialRelIntersects&outSR=4326&f=json") 


open_mines <- shapefile %>% 
...
ggplot() + 
  ...
  geom_sf(data=open_mines) +
  theme_minimal()
```

**A2.** The majority of the mines sit in the deep south of the state. In the previous map, the areas with the most loans per 100k people appear to be the same areas where mines are. 
