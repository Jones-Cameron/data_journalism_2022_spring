...
**Answer** Gives us a dataset with 5 fields representing states, voting, percentage of trump voters, businesses recievinf ppe and amount per employee as well as 51 rows, representing the 50 states plus DC.
```{r}
...
**Answer** A plot with a line of best fit. A bit hard to decipher what it means from the plot alone, but we can guess that the more trump voters the more loans. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** Much messier plot, hard to take anything from it. Very slight upward trend, but not enough to have a fuss about. 
```{r}
...
**Answer** See the correlation between trump percentage and businesses recieving loans, and loan amount per employee. 
```{r}
reveal_data %>%
...
**Answer** We get the correlatgion and p value. We know that our findings are likely to be accurate. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** Higher P value and weaker r value show less correlation.
```{r}
...
**Answer** mean loan amount per employee and mean businesses recieving loans pct between Trump and Biden voters. Trump leads in both categories. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** Sample T test, show we probably haven't found anything meaningful.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** Stronger T value and p value is extremely, showing a good correlation. 
```{r}
