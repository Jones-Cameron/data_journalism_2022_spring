...
**Answer**
There is the source code of the HTML that makes the page.
...
**Answer** Reads the source of the page not the entire data page
...
**Answer** Just the head and body code of the HTML, very little information
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** It gives you a better look at what HTML code you are specifically looking at. Helps better grab HTML tables. 
...
**Answer** Saves all tables in the HTML to naics industry rather than simply the HTML. Get a ist of 6 data frames now. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** You see 6 different data frames for the 6 different tables. 
...
**Answer** You can look at each individual dataframe. 
...
**Answer** the naics industry code becomes our one dataframe that we want rather than all 6.
...
**Answer** Went from 21 rows to 20 bc the 21st row wasn't actual data, cleaned header names
...
**Answer** Its the same as before. Cleaned and 20 rows. 
```{r}
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** Just the basic HTML, no table
...
**Answer** Calling the html element not html table. Pulls the header and footer for the one html part we want. 
...
**Answer** new dataframe we needed
...
**Answer** Row 2 remained, names were cleaned and information was sliced
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** constructing sector stats
...
**Answer** sectors all printed out
```{r}
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** sector name could not be found
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** sector numbers and descriptions
...
**Answer** 20 rows in the dataframe
...
**Answer** printed the 20 row numbers 1-20
...
**Answer** earlier table is loaded in 
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** sector and description dataframe is loaded
```{r}
# Define url of page we want to scrape
...
**Answer** grabbed new dataframe and merge
...
**Answer** filtered out a sector that had no page
...
**Answer** each rowis seperated into 19 different dataframes
...
**Answer** each individual dataframe is stored as a url and x path value to be referred to later
```{r}
...
**Answer** given many different tables
...
**Answer** extracted the dataframes from the nested list. got rid of information that was unnecessary
...
**Answer** now have a bunch of columns with necessary info 
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** each dataframe now only has 3 columns 
...
**Answer** the tables are cleaned and combined into one table
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** page of the mining sector andworkforce statistics
...
**Answer** same table, no agriculture
