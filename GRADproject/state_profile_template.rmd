...
colorado_county_pop <- colorado_county_pop %>% 
  ...
  mutate(project_county_name = toupper(str_remove_all(project_county_name, " County, Colorado"))
         ) %>% 
## not sure why im getting an error for this remove all function. "Error in stri_replace_all_regex(string, pattern, fix_replacement(replacement),  : object 'project_county_name' not found" Ive tried to google it, retype it, look at old labs and I can't seem to find the issue.
